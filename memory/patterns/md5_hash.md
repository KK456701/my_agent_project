---
name: "缓存键设计不安全且不合理"
description: "缓存键 cache_key = token[-20:] 只使用了 JWT 的后 20 个字符。这可能导致不同 Token 产生相同的缓存键（哈希冲突），从而返回错误的用户数据。同时，使用 Token 的一部分作为键，而不是使用 Token 的完整哈希或用户 ID，增加了安全风险。"
---
...
- **修复**: 使用 Token 的完整 SHA256 哈希作为缓存键，或者使用用户 ID + Token 的哈希组合。更安全的做法是使用 JWT 的 jti（JWT ID）声明作为缓存键，因为它是唯一的。
...
- **修复**: 改为 bcrypt.hashpw(password.encode(), bcrypt.gensalt())
...
- **修复**: 1. 使用完整的 token 哈希作为缓存 key。2. 为缓存添加 TTL（如 5 分钟），或使用 Redis 等支持过期时间的缓存。3. 如果必须缓存，应同时缓存 token 的过期时间并验证。
...
- **修复**: 1. 使用完整的 token SHA256 哈希作为缓存 key。2. 为缓存添加 TTL（如 5 分钟），或使用 Redis 等支持过期时间的缓存。3. 如果必须缓存，应同时缓存 token 的过期时间并验证。修改为：cache_key = hashlib.sha256(token.encode()).hexdigest(); 并检查 payload 中的 exp 字段。
...
- **修复**: 使用 Token 的完整 SHA256 哈希作为缓存键，或者使用 JWT 的 jti（JWT ID）声明作为缓存键。修改为：import hashlib; cache_key = hashlib.sha256(token.encode()).hexdigest()
...
- **修复**: 立即停止使用该库。短期方案：在调用 decrypt_payload 前增加输入验证和长度限制，并部署 WAF 规则拦截可疑请求。长期方案：按计划替换为安全库，同时考虑使用硬件安全模块（HSM）或 sidecar 代理来隔离风险。
...
- **修复**: 移除未使用的导入：删除 'import hashlib' 这一行。
...
- **修复**: 使用 JWT 的 jti（JWT ID）作为缓存键，并设置合理的 TTL（如 15 分钟）。同时使用 `cachetools.TTLCache` 替代手动字典，自动过期。
...
- **修复**: 使用 Token 的完整 SHA256 哈希作为缓存键，或者使用 JWT 的 jti（JWT ID）声明作为缓存键，因为它是唯一的。
...
- **修复**: 1) 拆分为多个独立的装饰器或中间件：@jwt_required @require_permission('admin') @audit_log @rate_limit；2) 使用 functools.lru_cache 或 Redis 替代手动字典缓存；3) 使用完整的 JWT 库（如 PyJWT）进行签名验证；4) 使用 logging 模块替代 print。
...
- **修复**: 使用 Token 的完整 SHA256 哈希作为缓存键，或者使用用户 ID + Token 的哈希组合。更安全的做法是使用 JWT 的 jti（JWT ID）声明作为缓存键，因为它是唯一的。同时应设置合理的 TTL。
...
- **修复**: 1. 使用 Token 的完整 SHA256 哈希作为缓存键。2. 移除无用的 HMAC 计算代码，或使用 PyJWT 库进行完整的签名验证。3. 如果必须缓存，应设置合理的 TTL 并考虑 Token 吊销场景。
...
- **修复**: 使用 Token 的完整 SHA256 哈希作为缓存键。正确实现 HMAC 签名验证：expected_signature = hmac.new(secret.encode(), token.encode(), hashlib.sha256).hexdigest(); if payload['signature'] != expected_signature: raise ValueError('Invalid signature')
...
- **修复**: 1. 使用 Token 的完整 SHA256 哈希作为缓存键。2. 使用 functools.lru_cache 或 cachetools.TTLCache 替代手动字典，设置合理的 TTL（如 5 分钟）。3. 移除无意义的 HMAC 计算。4. 使用 asyncio.sleep() 替代 time.sleep() 或在异步上下文中使用非阻塞限流。
...
- **修复**: 使用 Token 的完整 SHA256 哈希作为缓存键，或者使用 JWT 的 jti（JWT ID）声明作为缓存键。修改为：cache_key = hashlib.sha256(token.encode()).hexdigest()
...
- **修复**: 使用 Token 的完整 SHA256 哈希作为缓存键，或者使用用户 ID + Token 的哈希组合。更安全的做法是使用 JWT 的 jti（JWT ID）声明作为缓存键，因为它是唯一的。同时添加 TTL 过期策略。
...
- **修复**: 1. 使用 Token 的完整 SHA256 哈希或 JWT 的 jti 声明作为缓存键。2. 使用带 TTL 的缓存（如 cachetools.TTLCache）或 Redis 并设置合理的过期时间。3. 考虑使用线程安全的缓存实现。
...
- **来源 PR**: Demo: 用户登录模块
- **文件**: demo/sample_pr.py:204-213
- **描述**: 缓存键 cache_key = token[-20:] 只使用了 JWT 的后 20 个字符。这可能导致不同 Token 产生相同的缓存键（哈希冲突），从而返回错误的用户信息。同时，缓存没有过期时间，已吊销的 Token 仍然有效。
- **修复**: 使用 Token 的完整 SHA256 哈希作为缓存键，或者使用用户 ID + Token 的哈希组合。更安全的做法是使用 JWT 的 jti（JWT ID）声明作为缓存键，因为它是唯一的。同时添加 TTL 过期机制。

### 案例 29
- **日期**: 2026-05-19_103513
- **来源 PR**: Demo: 用户登录模块
- **文件**: demo/sample_pr.py:186-215
- **描述**: 缓存键 cache_key = token[-20:] 只使用了 JWT 的后 20 个字符。这可能导致不同 Token 产生相同的缓存键（哈希冲突），从而返回错误的用户权限信息。同时，缓存无过期时间，Token 被吊销后仍可使用。
- **修复**: 使用 Token 的完整 SHA256 哈希作为缓存键，或者使用用户 ID + Token 的哈希组合。更安全的做法是使用 JWT 的 jti（JWT ID）声明作为缓存键，因为它是唯一的。同时添加 TTL 过期机制。

### 案例 30
- **日期**: 2026-05-19_103843
- **来源 PR**: Demo: 用户登录模块
- **文件**: demo/sample_pr.py:212-220
- **描述**: _TOKEN_CACHE 使用 token[-20:] 作为缓存键，仅取 JWT 的后 20 个字符。这可能导致不同 Token 产生相同的缓存键（哈希冲突），返回错误的用户数据。同时，缓存没有 TTL 或淘汰策略，Token 过期后仍可能返回旧数据。
- **修复**: 1. 使用 Token 的完整 SHA256 哈希作为缓存键。2. 使用 cachetools.TTLCache 设置合理的 TTL（如 5 分钟）。3. 或者完全移除缓存，依赖 JWT 库的快速验证。修改为：from cachetools import TTLCache; _TOKEN_CACHE = TTLCache(maxsize=1000, ttl=300); cache_key = hashlib.sha256(token.encode()).hexdigest()。

### 案例 31
- **日期**: 2026-05-19_103843
- **来源 PR**: Demo: 用户登录模块
- **文件**: demo/sample_pr.py:198-206
- **描述**: 缓存键 cache_key = token[-20:] 只使用了 JWT 的后 20 个字符。这可能导致不同 Token 产生相同的缓存键（哈希冲突），从而返回错误的用户信息。同时，缓存没有过期时间，Token 吊销后缓存仍有效。
- **修复**: 使用 Token 的完整 SHA256 哈希作为缓存键，或者使用用户 ID + Token 的哈希组合。更安全的做法是使用 JWT 的 jti（JWT ID）声明作为缓存键，因为它是唯一的。同时添加 TTL 过期机制。

### 案例 32
- **日期**: 2026-05-19_104435
- **来源 PR**: stalemate_test.py
- **文件**: stalemate_test.py:24-30
- **描述**: process_secure_transaction 函数依赖 old-crypto-lib v1.2.3，该库存在 CVE-2024-5678（CVSS 9.8，远程代码执行）。每天 500 万次调用意味着攻击面极大。虽然替换成本高（重写加密层需 3 个月，性能降 15 倍），但当前状态属于不可接受的安全风险。
- **修复**: 短期：在 old-crypto-lib 外层增加输入验证和沙箱隔离，限制 crafted ciphertext 的破坏范围。长期：启动加密层重写计划，使用 pyca/cryptography 或 libsodium 替代，并利用硬件加速（如 Intel QAT）缩小性能差距。

### 案例 33
- **日期**: 2026-05-19_110427
- **来源 PR**: stalemate_test.py
- **文件**: stalemate_test.py:20
- **描述**: 代码中使用了 `old-crypto-lib v1.2.3`，该库存在已知的 CVE-2024-5678 (CVSS 9.8)，攻击者可以通过构造恶意的密文触发远程代码执行。`decrypt_payload` 函数直接处理外部输入的 `encrypted_data`，风险极高。
- **修复**: 立即替换 `old-crypto-lib` 为已修复或更安全的库（如 `cryptography`）。如果无法立即替换，应在解密前对 `encrypted_data` 进行严格的格式校验和完整性检查（如 HMAC），并部署 WAF 规则尝试拦截恶意载荷。
