...
- **建议**: 1. 如果 API Gateway 与后端在同一信任域内，可信任 Gateway 的校验结果，移除应用层重复校验。2. 如果必须保留深度防御，将校验逻辑合并为一次调用（如 _validate_all），减少函数调用开销。3. 使用缓存：对相同输入跳过重复校验。4. 将校验移至中间件层，避免每个业务函数重复实现。
...
- **建议**: 1. 移除应用层的重复校验，仅保留 Gateway 无法校验的业务逻辑规则（如库存检查、价格计算）。2. 如果深度防御是硬性要求，将校验逻辑合并为一次调用（如批量校验函数），减少函数调用开销。3. 使用缓存校验结果：对相同输入（如 item_name）的校验结果进行缓存，避免重复计算。
