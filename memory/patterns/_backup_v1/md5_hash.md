...
- **建议**: 使用 Token 的完整 SHA256 哈希作为缓存键，或者使用用户 ID + Token 的哈希组合。更安全的做法是使用 JWT 的 jti（JWT ID）声明作为缓存键，因为它是唯一的。同时添加 TTL 过期机制。
...
- **建议**: 1. 使用 Token 的完整 SHA256 哈希作为缓存键。2. 使用 cachetools.TTLCache 设置合理的 TTL（如 5 分钟）。3. 或者完全移除缓存，依赖 JWT 库的快速验证。修改为：from cachetools import TTLCache; _TOKEN_CACHE = TTLCache(maxsize=1000, ttl=300); cache_key = hashlib.sha256(token.encode()).hexdigest()。
...
- **建议**: 短期：在 old-crypto-lib 外层增加输入验证和沙箱隔离，限制 crafted ciphertext 的破坏范围。长期：启动加密层重写计划，使用 pyca/cryptography 或 libsodium 替代，并利用硬件加速（如 Intel QAT）缩小性能差距。
...
- **建议**: 立即替换 `old-crypto-lib` 为已修复或更安全的库（如 `cryptography`）。如果无法立即替换，应在解密前对 `encrypted_data` 进行严格的格式校验和完整性检查（如 HMAC），并部署 WAF 规则尝试拦截恶意载荷。
